...
def getData():
        review_train = read_csv("rotten_tomatoes_critic_reviews_train.csv", sep=",", encoding="ISO-8859-1")
        review_train = review_train.rename(index=str,
                                             columns={"rotten_tomatoes_link": "id", "review_type": "sentiment", "review_content": 
                                                      "text"})
        return review_train
...
if __name__ == '__main__':
        ...
        ret = model.predict(["So let's recap: We've broken into Buckingham Palace, and the Oval Office, stolen a page from the President's super-secret book, and actually kidnapped the President of the United States. What are we gonna do next, short-sheet the Pope's bed?"])[-1]
        print(ret)
